...
* Go to <https://github.com/adobe/wknd-headless/releases/latest> and download `wknd-headless-{version}.zip`
* Upload `wknd-headless-{version}.zip` in AEMaaCS's site creation wizard to create a new site from that template.

## Build locally

Alternatively you can build `wknd-headless-{version}.zip` locally.
...
1. The site template ZIP file is now located below the template root: `wknd-headless-{version}.zip`.
...
* Commit, build and release latest Site Theme related changes in [Site Theme E2E Repo](https://github.com/adobe/wknd-headless-theme-e2e).
